...
4. 在“完整行程”添加航班、自驾、住宿、活动或接驳事项。填写当地日期时间、时区、真实起终点、来源、提醒及关联文件。跨时区事项支持分别设置开始和结束时区。首页自动显示当前或下一事项及倒计时。单个事项可以在详情标记“已结束”或“已取消”，首页跳过这些安排，完整行程保留状态标签；详情可以恢复，资料与账本不会删除，也不会改变实际预订。
