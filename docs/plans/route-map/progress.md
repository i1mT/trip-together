...
- 2026-09-22 第十一次更新：播放到新的一天时，底部渐显渐隐地显示「第一天 10.1」小卡片（天序按行程开始日算，日期为月.日），2.8 秒后自动淡出；用 `animation` + 按当天日期做 key，切天时自然重播动画，停止播放即移除。新增 e2e 断言首日与第三天卡片文案。`Journey`/`buildJourney` 已移到 `route-geometry.ts`，单文件仍不超过 400 行。
