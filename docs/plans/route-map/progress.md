...
- 2026-09-22 第十次更新：解决播放时底图瓦片还没加载完就开始前进、录到空白瓦片的问题。播放时间线改为可冻结：每段镜头就位后先跑 120ms 让新视口的瓦片请求发出，随后只要 `map.areTilesLoaded()` 为假就冻结时间线（载具、线段、进度条都不前进），瓦片加载完立即继续；每段最多等 5 秒，超时继续以免网络异常卡死。用 CDP 限速实测：限到 12Mbps/80ms 时城市级缩放的空白瓦片消失，在 1Mbps 极端限速下超出 5 秒上限仍会前进（已知边界）。同时把 `Journey`/`buildJourney` 移到 `route-geometry.ts`，保持单文件不超过 400 行。
