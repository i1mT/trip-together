...
const UNSUPPORTED = [
  ["class static block（class static block，Safari 16.4+）", /(?:^|[;{}(\s])static\s*\{/],
  ["static private method（Safari 16.4+）", /static\s+#[\w$]+\s*\(/],
  ["static accessor（Safari 17.4+）", /(?:^|[;{}\s])accessor\s/],
  ["explicit resource management（using，Safari 18+）", /(?:^|[;{}\s])using\s+[\w$]+\s*=/],
  ["await using（Safari 18+）", /await\s+using\s/],
  ["decorator（Safari 17.4+）", /(?:^|\n)\s*@[A-Za-z_$][\w$]*(?:\s*\([^)\n]*\))?\s*\n\s*(?:export\s+)?class\s/],
  ["RegExp v flag（Safari 17+）", /(?:^|[=(,:;[!&|?{}\s])\/[^/\n]*\/[dgimsuvy]*v[dgimsuvy]*[;,)\]}.\s]/],
  ["RegExp duplicate named group（Safari 17.4+）", /\(\?<([A-Za-z_$][\w$]*)>[\s\S]{0,200}?\(\?<\1>/],
  ["Array.prototype.toSorted / toReversed / toSpliced / with（Safari 16.4+）", /\.(?:toSorted|toReversed|toSpliced)\(|\.with\(/],
  ["Array.fromAsync（Safari 18.4+）", /Array\.fromAsync\b/],
  ["Object.groupBy / Map.groupBy（Safari 17.4+）", /\b(?:Object|Map)\.groupBy\b/],
...
console.log(`浏览器兼容检查通过：${files.length} 个 chunk 均可在 iOS 15.5 Safari 解析。`);
